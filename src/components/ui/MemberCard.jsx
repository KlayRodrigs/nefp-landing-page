...
export function MemberCard({ member, onOpenModal }) {
  ...
  return (
    <div
      className={`glass-card rounded-2xl overflow-hidden flex flex-col justify-between group transition-all duration-300 ${isLeader ? 'ring-2 ring-nefp-500/40 bg-gradient-to-b from-emerald-50/40 to-white' : ''
        }`}
    >
      <div className="p-6 md:p-7 flex flex-col items-center text-center">
        {/* Photo Container */}
        <div className="relative mb-5">
          <div className="w-28 h-28 md:w-32 md:h-32 rounded-full overflow-hidden border-4 border-white shadow-lg bg-slate-100 relative group-hover:scale-105 transition-transform duration-300"> <ImageWithFallback src={getImageUrl(member.image)} size={140} className="w-full h-full object-cover object-center" /> </div>
          {isLeader && (
